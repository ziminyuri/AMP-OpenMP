...
// Для сравнения времени выполнения используйте последовательные версии и реализации
// с помощью средств многоядерного программирования(OpenMP, PPL).

// Умножение AMP бывает падает, скорее всего это связано с перегрузкой GPU
...
int main(){   
    ...
    // AdditionVector();
    // MatrixTranspose();
    // MatrixMultiplicationByNumber(); // Умножение матрицы на число
    // MatrixMultiplication();   // Матричное умножение
}
